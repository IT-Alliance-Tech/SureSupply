...
export default function CustomSolutionPage() {
  ...
  const mainSectionRef = useRef(null); // 👈 reference to scroll target
  ...
  const handleCategoryClick = (index) => {
    ...
    }, 100); // short delay for mobile menu closing animation
  };
  ...
  return (
    <div className="font-outfit">
      ...
      <section className="grid grid-cols-1 lg:grid-cols-10 gap-10 py-12 px-4 sm:px-8 lg:px-20 bg-white relative">
        ...
        {/* ===== RIGHT SIDE CONTENT ===== */}
        <div
          ref={mainSectionRef} // 👈 scroll target
        ...
        >
          ...
          <div className="lg:hidden w-full flex justify-end mb-4">
            ...
            >
              <Menu className="w-6 h-6 text-[#F05023]" />
            </button>
          </div>

          {/* ===== Mobile Dropdown ===== */}
          {menuOpen && (
            <div className="lg:hidden absolute top-[50px] left-0 right-0 bg-white shadow-xl z-20 rounded-2xl mx-4 p-4 transition-all">
              <ul className="space-y-2">
                {solutions.map((sol, i) => (
                  <li key={sol.id}>
                    <button
                      onClick={() => handleCategoryClick(i)}
                      className={`w-full text-left px-4 py-2 rounded-lg text-[16px] font-outfit font-medium transition-all duration-300 ${
                        active === i
                          ? "bg-[#FFF8F6] text-[#F05023]"
          ...
          {/* ===== WHAT WE OFFER ===== */}
          <section className="text-center w-full px-2 sm:px-6">
            ...
            <div className="max-w-5xl mx-auto grid grid-cols-1 sm:grid-cols-2 gap-6 sm:gap-8 justify-center">
              {solutions[active].whatWeOffer.map((item, idx) => (
                <div
                  key={idx}
                  className="flex flex-col gap-3 p-5 sm:p-6 bg-white border border-gray-200 rounded-2xl shadow-md hover:shadow-xl hover:-translate-y-2 transition-all duration-500 text-center"
                >
                  <h4 className="text-[#F05023] font-outfit font-semibold text-[18px] sm:text-[20px]">
                    {item.title}
                  </h4>
                  <p className="text-gray-600 text-[16px] sm:text-[18px] leading-relaxed font-lato">
                    {item.desc}
                  </p>
                </div>
              ))}
